...
print(my_data_dict)
try:
    word_data=pandas.read_csv("data/word_to_learn.csv")
...
def gen_french_word():
    ...
    global fliptimer,count,french_word_dic
...
def isknown():
        my_data_dict.remove(french_word_dic)  # remove that data from my french_worddictlist
        #print(len(my_data_dict))
        data=pandas.DataFrame(my_data_dict)
...
tick_button=Button(image=tick_image,highlightthickness=0,command=isknown)
...
#task5 as userr click \/ than remove that data from listof dictionary
#list.remove(val)